...
* ob, ssd, aud, pter, ttg, hds, hdc 等站列表页有IMDb标识，torfilter会用IMDb来查重并在推送qbit下载时添加标签(tag)
...
* 在“查&下”过程中，每次下载后有约2秒的间隔，在“仅查重”时则无；


### Note:
* 部分站不支持国语，中字标签搜索
* ob, chd, ssd, frds, hds 的下载链接无passkey，拼合usercp中的passkey构成下载链接
* 种子列表页面上无法取得imdb的(frds, chd, beitai)，以及列表页上无法取得下载链接的(hds)，会进入种子详情页获取信息，这会对站点发起拉取页面的操作，请自行把握。
